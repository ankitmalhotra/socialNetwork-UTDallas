	...
	for( cur = cur->children ; cur != NULL ; cur = cur->next ) {
		if(cur->type == XML_ELEMENT_NODE) {
			...
			if(xmlStrEqual(cur->name, (const xmlChar *) "return")) {
				...
				xmlChar *instanceType = xmlGetNsProp(cur, (const xmlChar *) "type", (const xmlChar *) "http://www.w3.org/2001/XMLSchema-instance");
				if(instanceType == NULL) {
					elementClass = [ax22_GroupDetails class];
				} else {
					NSString *elementTypeString = [NSString stringWithCString:(char*)instanceType encoding:NSUTF8StringEncoding];
...
+ (void)initialize
{
	...
	[[USGlobals sharedInstance].wsdlStandardNamespaces setObject:@"ax25" forKey:@"http://sql.java/xsd"];
...
{
	return [[[GroupsDataServiceServiceSoap11Binding alloc] initWithAddress:@"http://localhost:8080/GroupMessagingApp/services/GroupsDataServiceService.GroupsDataServiceServiceHttpSoap11Endpoint/"] autorelease];
}
+ (GroupsDataServiceServiceSoap12Binding *)GroupsDataServiceServiceSoap12Binding
{
	return [[[GroupsDataServiceServiceSoap12Binding alloc] initWithAddress:@"http://localhost:8080/GroupMessagingApp/services/GroupsDataServiceService.GroupsDataServiceServiceHttpSoap12Endpoint/"] autorelease];
}
@end
...
- (NSString *)serializedFormUsingHeaderElements:(NSDictionary *)headerElements bodyElements:(NSDictionary *)bodyElements
{
	...
	xmlNewNs(root, (const xmlChar*)"http://sql.java/xsd", (const xmlChar*)"ax25");
